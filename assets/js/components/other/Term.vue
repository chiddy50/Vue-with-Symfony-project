                ...
                <tr class="text-center" v-for="(term, index) in terms" :key="index">
                  ...
                  <td class="text-center">
                    <button
                      ...
                      @click="deleteTerm(term.id, index, $event)"
                    >
        ...
        Axios.post("/edit-term", fd)
          .then(response => {
            ...
            if (data.error) {
              ...
              self.$store.state.terms = data.terms;
            }
          })
          .catch(err => {
            console.log(err);
          ...
          .finally(() => {
            self.termLoading = false;
            self.resetForm();
          });
      } else {
        self.termLoading = true;
        let fd = new FormData(e.target);
        Axios.post("/new-term", fd)
          .then(response => {
            ...
            if (data.error) {
              ...
              self.$store.state.terms = [...self.$store.state.terms, data.term];
              self.code = "";
              self.description = "";
            }
          })
